...
void list_foldl (void (*f) (element_t*, element_t, element_t), element_t* out_element_p,  struct list* in_list) {
  ...
  if (out_element_p == NULL) {
    element_t newout = malloc(sizeof(element_t));
    out = newout;
  } else {
    out = out_element_p;
  }
...
void list_filter (int (*f) (element_t), struct list* out_list, struct list* in_list) {
  // TODO
  int acc = 0;
  for(int i=in_list->len ; i>=0 ; i--) {
    if(f(in_list->data[i])){
      list_insert (out_list, acc, in_list->data[i]);
    }
  }
}
